...
function solve(solver::BIM_hyperbolic,basis::Ba,pts_hyp::BoundaryPointsHyp,k;multithreaded::Bool=true,use_krylov::Bool=true,tol=1e-12,maxiter::Int=2000) where {Ba<:AbstractHankelBasis}
    ...
    isnothing(symmetry) ? compute_kernel_matrices_DLP_hyperbolic!(K,pts,tab;multithreaded=multithreaded) : compute_kernel_matrices_DLP_hyperbolic!(K,pts,symmetry,tab;multithreaded=multithreaded)
...
function d_bounds_hyp(bp::BoundaryPointsHyp{T},symmetry;K::Int=3,dmin_floor::T=T(1e-6),pad_min=T(0.9),pad_max=T(1.1),ρ_extra::T=zero(T)) where{T<:Real}
    ...
    dmax=pad_max*_dmax_from_rho(ρ);dmin=typemax(T)
    ...
    @inbounds for i in 1:N
        xi=xy[i][1];yi=xy[i][2]
        for t in 1:K
            for j in (_wrap(i+t,N),_wrap(i-t,N))
                x0=xy[j][1];y0=xy[j][2];upd!(xi,yi,x0,y0)
                if !isnothing(symmetry)
                    s=symmetry
                    if s isa Reflection
                        if s.axis==:origin
                            upd!(xi,yi,-x0,y0);upd!(xi,yi,x0,-y0);upd!(xi,yi,-x0,-y0)
                        elseif s.axis==:y_axis
                            upd!(xi,yi,-x0,y0)
                        elseif s.axis==:x_axis
                            upd!(xi,yi,x0,-y0)
                        end
                    elseif s isa Rotation
                        cx=s.center[1];cy=s.center[2];ctab,stab,_=_rotation_tables(T,s.n,mod(s.m,s.n));x=x0-cx;y=y0-cy
                        for l in 2:s.n

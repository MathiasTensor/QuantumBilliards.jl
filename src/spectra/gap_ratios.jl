...
function plot_average_r_vs_parameter!(ax::Axis,vec_energies::Vector{Vector{T}},μ_cs::Vector{T},x_axis_points::Vector{K}) where {T<:Real,K<:Real}
    ...
    hlines!(ax,[r_integ],color=:red,linestyle=:dash,linewidth=2,label=L"\langle r \rangle \; Poisson")
    hlines!(ax,[r_chaot],color=:black,linestyle=:dash,linewidth=2,label=L"\langle r \rangle \; GOE \; 3 \times 3")
    hlines!(ax,[r_chaot_corr],color=:green,linestyle=:dash,linewidth=2,label=L"\langle r \rangle \; GOE \; N \times N")

...
function plot_nnls_only_chaotic(chaotic_levels::Vector;nbins::Int=100,ρ_chaotic_classic=1.0)
    ...
    ax=Axis(f[1,1],title="NNLS distribution of chaotic levels upon separation by classical criterion")
...
function fit_br_tunneling_distorted_sigma(bin_centers::Vector, bin_counts::Vector, rho::T) where {T<:Real}
    ...
    init_params = [0.1]
    fit_result = curve_fit((bin_centers, params) -> tunneling_model(bin_centers, params), bin_centers, bin_counts, init_params)
    return fit_result.param[1]
end
...
function plot_nnls(unfolded_energies::Vector; nbins::Int=200, rho::Union{Nothing, T}=nothing, fit_brb::Bool=false, fit_only_beta=false, log_scale=false, fited_rho::Union{Nothing, T} = nothing, plot_tunneling_distorted_brb=false, plot_standard_error::Bool=false, plot_GUE=false) where {T <: Real}
    ...
    bin_std_devs = [std(spacings[(spacings .>= hist.edges[1][i]) .& (spacings .< hist.edges[1][i+1])]) / sqrt(count(x -> (x >= hist.edges[1][i]) && (x < hist.edges[1][i+1]), spacings)) for i in 1:(length(hist.edges[1]) - 1)]
...
"""
function plot_subtract_level_counts_from_weyl(arr::Vector{T}, billiard::Bi; bin_size::T = T(20.0), fundamental::Bool=true, offset=0.0, scale=1.0) where {Bi<:AbsBilliard, T<:Real} 
    A = T(fundamental ? billiard.area_fundamental : billiard.area)
    L = T(fundamental ? billiard.length_fundamental : billiard.length)
    C = T(curvature_and_corner_corrections(billiard; fundamental=fundamental))
    ...
    ys = [count(_k -> _k < k, arr) for k in arr]
    Ns = [(A/(4*pi)*k^2 - L/(4*pi)*k + C) for k in arr]
    new_ys = [scale*(y-N+offset) for (y, N) in zip(ys, Ns)]

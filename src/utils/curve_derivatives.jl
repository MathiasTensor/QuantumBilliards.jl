"""
    tangent(crv::BilliardGeometry.AbsCurve,ts::AbstractArray)
...
## Arguments
* `crv::BilliardGeometry.AbsCurve`: Boundary curve.
...
"""
    tangent_2(crv::BilliardGeometry.AbsCurve,ts::AbstractArray)
...
"""
    tangent(line::L,t) where {L<:BilliardGeometry.AbsLine}
...
"""
    tangent_2(line::L,t) where {L<:BilliardGeometry.AbsLine}
...
"""
    tangent(circle::L,t) where {L<:BilliardGeometry.CircleSegment}
...
"""
    tangent_2(circle::L,t) where {L<:BilliardGeometry.CircleSegment}
...
"""
    _polar_radius_derivative(polar::L,phi::T) where {L<:BilliardGeometry.PolarSegment,T<:Real} → T

Return the first angular derivative `dr/dphi` of the Fourier radius stored by a
`BilliardGeometry.PolarSegment`.
...
"""
@inline function _polar_radius_derivative(polar::L,phi::T) where {L<:BilliardGeometry.PolarSegment,T<:Real}
    dr=zero(T)
...
"""
    _polar_radius_derivative_2(polar::L,phi::T) where {L<:BilliardGeometry.PolarSegment,T<:Real} → T

Return the second angular derivative `d²r/dphi²` of the Fourier radius stored by
a `BilliardGeometry.PolarSegment`.

The derivative is evaluated analytically from the Fourier coefficients.
"""
@inline function _polar_radius_derivative_2(polar::L,phi::T) where {L<:BilliardGeometry.PolarSegment,T<:Real}
    ddr=zero(T)
    sin_coef=polar.coef[1:2:end]
    cos_coef=polar.coef[2:2:end]
    @inbounds for (n,a) in enumerate(cos_coef)
...
"""
    tangent(polar::L,t::T) where {L<:BilliardGeometry.PolarSegment,T<:Real}
...
* `dr`: First parameter derivative `dr/dt`.
"""
@inline function tangent(polar::L,t::T) where {L<:BilliardGeometry.PolarSegment,T<:Real}
    phi=polar.shift_angle+t*polar.arc_angle
    r=BilliardGeometry.polar_radius(polar,phi)
...
"""
    tangent_2(polar::L,t::T) where {L<:BilliardGeometry.PolarSegment,T<:Real}
...
* `ddr`: Second parameter derivative `d²r/dt²`.
"""
@inline function tangent_2(polar::L,t::T) where {L<:BilliardGeometry.PolarSegment,T<:Real}
    phi=polar.shift_angle+t*polar.arc_angle
    r=BilliardGeometry.polar_radius(polar,phi)
    dr=_polar_radius_derivative(polar,phi)
...
"""
    tangent_vec(crv::L,ts::AbstractVector{T}) where {T<:Real,L<:BilliardGeometry.AbsCurve}
...
"""
function tangent_vec(crv::L,ts::AbstractVector{T}) where {T<:Real,L<:BilliardGeometry.AbsCurve}
    ta=tangent(crv,ts)
    return [ti/norm(ti) for ti in ta]
end

"""
    normal_vec(crv::L,ts::AbstractVector{T}) where {T<:Real,L<:BilliardGeometry.AbsCurve}
...
"""
function normal_vec(crv::L,ts::AbstractVector{T}) where {T<:Real,L<:BilliardGeometry.AbsCurve}
    ta=tangent_vec(crv,ts)
    return [SVector(ti[2],-ti[1]) for ti in ta]
end

"""
    curvature(crv::L,ts::AbstractVector{T}) where {T<:Real,L<:BilliardGeometry.AbsCurve} → Vector{T}
...
"""
function curvature(crv::L,ts::AbstractVector{T}) where {T<:Real,L<:BilliardGeometry.AbsCurve}
    dr=tangent(crv,ts)
...
"""
    curvature(crv::L,t::T) where {T<:Real,L<:BilliardGeometry.AbsCurve} → T
...
"""
function curvature(crv::L,t::T) where {T<:Real,L<:BilliardGeometry.AbsCurve}
    dr=tangent(crv,t)
    ddr=tangent_2(crv,t)

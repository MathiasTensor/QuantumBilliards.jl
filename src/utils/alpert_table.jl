...
Container for one tabulated Alpert quadrature rule for logarithmic
singularities.

## Description
`AlpertLogRule` stores the shifted correction nodes and quadrature weights used
by the hybrid Gauss-trapezoidal rule of Alpert for logarithmically singular
integrals.

For a target node, the ordinary trapezoidal contribution is omitted in a local
near region of width `a` and replaced by a weighted sum evaluated at the shifted
nodes `x`.

## Attributes
* `order`: Formal order of the Alpert correction rule.
* `a`: Width of the near region excluded from the ordinary trapezoidal sum.
* `j`: Number of shifted correction nodes.
* `x`: Shifted node locations measured in units of the local mesh spacing.
* `w`: Quadrature weights associated with the shifted nodes.

## Notes
The rule is used primarily for self-interactions of logarithmically singular
kernels such as the two-dimensional Helmholtz single-layer potential.
"""
...
"""
    alpert_log_rule(::Type{T},order::Int) where {T<:Real} → rule::AlpertLogRule{T}

Returns a tabulated Alpert quadrature rule for logarithmically singular
integrals.

## Description
The returned rule implements the hybrid Gauss-trapezoidal corrections tabulated
in Alpert, *Hybrid Gauss-Trapezoidal Quadrature Rules*, SIAM J. Sci. Comput.
20(5), 1551-1584 (1999).

The ordinary trapezoidal rule is used away from the singular target. In a local
near region, the ordinary nodes are omitted and replaced by shifted correction
nodes with specially chosen weights.

The supported formal orders are

    2,3,4,5,6,8,10,12,14,16.

## Arguments
* `T`: Real scalar type used for the tabulated nodes and weights.
* `order`: Requested formal order of the logarithmic Alpert rule.

## Returns
* `rule`: [`AlpertLogRule`](@ref) containing the correction width `a`, number of
  shifted nodes `j`, shifted node locations `x`, and corresponding weights `w`.

## Throws
* `ArgumentError`: If `order` is not one of the tabulated orders.
"""

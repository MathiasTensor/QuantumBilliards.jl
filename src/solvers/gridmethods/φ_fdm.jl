...
# ROBNIK
#=
function robnik_psi(x,y,ε)
    θ=atan(y,x)+pi
    r=1.0+ε*cos(θ)
    return x^2+y^2<r^2
end
phi(x,y)=robnik_psi(x,y,0.2)
...
fem=QuantumBilliards.FiniteElementMethod(billiard,300,300;k_max=1000.0,offset_x_symmetric=0.1,offset_y_symmetric=0.1,fundamental=fundamental) # NEVER use 0.0 offsets in φ-FDM since it will fail since in it requires exterior cells that are adjacent to the interior.
...
save("MASK.png",f) # check if the mask is working correctly

...
function basis_matrix(basis::Ba, k, pts::Vector{SVector{2,T}}; multithreaded = true) where {T<:Real, Ba<:AbsBasis}
    let dim = basis.dim
        B = basis_fun(basis,1:dim,k,pts; multithreaded)
        return filter_matrix!(B)
    end
end

function gradient_matrices(basis::Ba, k, pts::Vector{SVector{2,T}}; multithreaded = true) where {T<:Real, Ba<:AbsBasis}
    let dim = basis.dim
        dB_dx, dB_dy = gradient(basis,1:dim,k,pts; multithreaded)
        return filter_matrix!(dB_dx), filter_matrix!(dB_dy)
    end
end

function basis_and_gradient_matrices(basis::Ba, k, pts::Vector{SVector{2,T}}; multithreaded = true) where {T<:Real, Ba<:AbsBasis}
    let dim = basis.dim
        B, dB_dx, dB_dy = basis_and_gradient(basis,1:dim,k,pts; multithreaded)
        return filter_matrix!(B), filter_matrix!(dB_dx), filter_matrix!(dB_dy)
    end
end


function dk_matrix(basis::Ba, k, pts::Vector{SVector{2,T}}; multithreaded = true) where {T<:Real, Ba<:AbsBasis}
    let dim = basis.dim
        dB_dk = dk_fun(basis,1:dim,k,pts; multithreaded)

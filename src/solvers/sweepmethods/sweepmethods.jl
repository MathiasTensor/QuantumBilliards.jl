...
- `tol::Real=1e-10`: Tolerance for numerical rank determination in the rank-reduced solver (ParticularSolutionsMethod). Adjust if necessary (you see too much spiking in the tension plot). A good choice is to start with 1e-10 and lower it if necessary. Rule of thumb here is that tol should increase with k since R[1,1] decreases with k.
...
"""
function k_sweep(solver::SweepSolver,basis::AbsBasis,billiard::AbsBilliard,ks;kernel_fun::Union{Symbol,Function}=:default,multithreaded_matrices::Bool=true,multithreaded_ks=false,use_combined::Bool=false,use_krylov::Bool=true,tol=1e-10) 
    k=maximum(ks)
    dim=max(solver.min_dim,round(Int,billiard.length*k*solver.dim_scaling_factor/(2*pi)))
    new_basis=resize_basis(basis,billiard,dim,k)
    pts=evaluate_points(solver,billiard,k)
...
function refine_minima(solver::SweepSolver,basis::AbsBasis,billiard::AbsBilliard,ks::AbstractVector{T},tens::AbstractVector{T};kernel_fun::Union{Symbol,Function}=:default,multithreaded_matrices::Bool=true,multithreaded_ks=false,use_combined::Bool=false,threshold=200.0,print_refinement=true) where {T<:Real}
    ...
    if solver isa BoundaryIntegralMethod
        f_min=k->begin
            pts=evaluate_points(solver_new,billiard,k)
            solve(solver_new,basis,pts,k;multithreaded=multithreaded_matrices,kernel_fun=kernel_fun)
        end
    elseif solver isa CFIE_polar_nocorners || solver isa CFIE_polar_corner_correction
        f_min=k->begin 
            pts=evaluate_points(solver_new,billiard,k)
            solve(solver_new,basis,pts,k;multithreaded=multithreaded_matrices,use_combined=use_combined)
        end
    else
        f_min=k->begin 
            dim=max(solver_new.min_dim,round(Int,billiard.length*k*solver_new.dim_scaling_factor/(2*pi)))
            new_basis=resize_basis(basis,billiard,dim,k)
            pts=evaluate_points(solver_new,billiard,k)
            solve(solver_new,new_basis,pts,k;multithreaded=multithreaded_matrices)
        end
    end

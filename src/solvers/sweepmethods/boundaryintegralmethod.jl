...
function solve(solver::ExpandedBoundaryIntegralMethod,basis::Ba,pts::BoundaryPointsBIM,k,dk;eps=1e-15) where {Ba<:AbstractHankelBasis}
    ...
    tens=abs.(λ) # similar to scaling method but since these are smallest
    ...
    denominators = real.(sum(conj(VL) .* (dA * VR), dims=1))[:]  # Flatten to 1D
    println("typeof denominators: ", typeof(denominators))
    numerators = real.(sum(conj(VL) .* (ddA * VR), dims=1))[:]  # Flatten to 1D 

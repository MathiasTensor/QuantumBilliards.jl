...
"""
    generalized_eigen_all_LAPACK_LEGACY(A::AbstractMatrix, B::AbstractMatrix) → (λ::Vector{Complex{T}}, VR::Matrix{Complex{T}}, VL::Matrix{Complex{T}}) where T <: Real
...
"""
    generalized_eigen_symmetric_LAPACK_LEGACY(A::AbstractMatrix, B::AbstractMatrix) → (λ::Vector{Real}, VR::Matrix{Complex{T}}, VL::Matrix{Complex{T}}) where T <: Real
...
## Arguments
* `A`: Symmetric or Hermitian square matrix. DON'T USE `Symmetric(...)` or `Hermitian(...)` since LAPACK does not support it, use `Matrix(...)`
* `B`: Symmetric or Hermitian square matrix. DON'T USE `Symmetric(...)` or `Hermitian(...)` since LAPACK does not support it, use `Matrix(...)`

## Returns
* `λ`: Vector of ordered filtered eigenvalues (excluding `NaN` and `Inf` values).
* `VR`: Complex matrix where each column is a right eigenvector.
* `VL`: Same as `VR`, since left eigenvectors = right eigenvectors for symmetric matrices.
"""

...
#################################################################
# Smaller score = better root.
#
# We combine:
#   1. imaginary-part magnitude
#   2. tension
#
# using logarithmic scaling so that both quantities contribute
# similarly over many orders of magnitude.
#
# True physical roots typically satisfy (based on the dk chosen ofc):
#
#   abs(imag(k)) ~ 1e-9 -- 1e-14
#
# while spurious duplicates generated by overlapping EBIM
# windows often satisfy:
#
#   abs(imag(k)) ~ 1e-6 -- 1e-5
#
# Lower tensions are also preferred.
# The logarithmic form prevents either quantity from completely
# dominating the score numerically.
#################################################################
...
###############################################################
# Estimate the local physical eigenvalue spacing around index i.
#
# A moving local window is used instead of a global spacing
# estimate because:
#
#   - spectral density changes with k
#   - EBIM intervals may span large k ranges
#   - duplicate roots create tiny artificial gaps
#
# We use the median gap for robustness against:
#
#   - accidental near-duplicates
#   - numerical outliers
#   - clustered spurious roots
###############################################################
function _local_gap(xs,i;w::Int=4)
    ...
    gaps=diff(xs[i1:i2])
    gaps=gaps[gaps.>0]
    isempty(gaps) && return Inf
    return median(gaps)
end

###########################################################
#
# Merge overlapping EBIM spectra generated from neighboring
# k-windows.
# Adaptive local-spacing criterion:
#
#   Δk < min(tolmax,spacing_frac*local_spacing)
#
# where:
#
#   Δk            = neighboring root separation
#   local_spacing = nearby physical eigenvalue spacing
#
# This makes clustering automatically scale with the local
# spectral density.
#
# Inside each cluster we use the root minimizing:
#
#   _ebim_scoring_logic(k,t)
#
# which strongly favors:
#
#   - smaller imaginary parts
#   - smaller tensions
#
# This removes spurious duplicate EBIM roots while preserving
# genuine nearby physical eigenvalues.
# ###########################################################
function overlap_and_merge_ebim!(k_left::Vector{K},ten_left::Vector{T},k_right::Vector{K},ten_right::Vector{T},control_left::Vector{Bool},kl::T,kr::T;tol::T=T(1e-5),spacing_frac::T=T(0.02),tolmax::T=T(5e-3),local_window::Int=4) where {K<:Number,T<:Real}
    ...
    while i<=length(k_all)
        ...
        block=i:j
        best=block[1]
        best_score=_ebim_scoring_logic(k_all[best],ten_all[best])
        for q in block
            score=_ebim_scoring_logic(k_all[q],ten_all[q])
            if score<best_score
                best=q
                best_score=score
            end
...
"""
    compute_spectrum(solver::EBIMSolver,billiard::Bi,k1::T,k2::T;dk=(k)->0.05*k^(-1/3),tol=1e-5,use_lapack_raw=false,multithreaded_matrices=false,use_krylov=true,seg_reuse_frac=0.95,solve_info=true,use_chebyshev=false,n_panels=15000,M=5) -> Tuple{Vector{T},Vector{T}}
...
NOTE v0.1: Is designed for desymmetrized geometries due to merging logic flagging degeneracies as tension check candidates for removal. Usually still works for symmetric cases but was not properly tested!
...
- `tol::T=1e-5`: Tolerance for merging overlapping eigenvalues.

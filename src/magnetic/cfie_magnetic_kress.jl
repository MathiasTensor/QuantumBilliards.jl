...
zᵢⱼ=r²ᵢⱼ/B².
"""
@inline function _mag_raw_regular_slp(tab,G::MagneticKressGeomCache{T},bmag::T,i::Int,j::Int) where {T<:Real}
    z=Float64(G.r2[i,j]/(bmag*bmag))
    return G.phase[i,j]*_eval_F(tab,z)*G.wsrc[j]
end
...
source normal.
"""
@inline function _mag_raw_regular_dlp_src(tab,G::MagneticKressGeomCache{T},bmag::T,i::Int,j::Int) where {T<:Real}
    z=Float64(G.r2[i,j]/(bmag*bmag))
    F=_eval_F(tab,z)
    Fz=_eval_Fz(tab,z)
...
"""
@inline function _mag_kress_slp(tab,G::MagneticKressGeomCache{T},bmag::T,i::Int,j::Int) where {T<:Real}
    if i==j
...
-Alog·curvnum/|γσ|².
"""
@inline function _mag_kress_dlp_src(tab,G::MagneticKressGeomCache{T},bmag::T,i::Int,j::Int) where {T<:Real}
    if i==j
        A=Complex{T}(tab.a_log)

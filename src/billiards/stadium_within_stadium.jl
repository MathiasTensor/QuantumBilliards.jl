# Stadium billiard with optional hole, constructed as a union of two stadiums (one for the outer boundary and one for the inner boundary if the hole is present).
...
function make_quarter_stadium_component(half_width;radius=one(half_width),x0=zero(half_width),y0=zero(half_width),rot_angle=zero(half_width))
    ...
    corners=[
        ...
        SVector(half_width+radius,zero(T))
    ]
...
function StadiumWithOptionalHole(outer_half_width;outer_radius=one(outer_half_width),inner_half_width=nothing,inner_radius=nothing,x0=zero(outer_half_width),y0=zero(outer_half_width))
    ...
    outer_fund,_=make_quarter_stadium_component(outer_half_width;radius=outer_radius,x0=x0,y0=y0)
    ...
    if has_hole
        ...
        inner_fund,_=make_quarter_stadium_component(ihw;radius=ir,x0=x0,y0=y0)
        full_boundary=[outer_full,inner_full]
        desymmetrized_full_boundary=[outer_desym,inner_desym]
        fundamental_boundary=[outer_fund,inner_fund]
        outer_area=4*outer_half_width*outer_radius+pi*outer_radius^2
        inner_area=4*ihw*ir+pi*ir^2
        ...
        length_fundamental=symmetry_accounted_fundamental_boundary_length(outer_fund)+symmetry_accounted_fundamental_boundary_length(inner_fund)
        inner_half_width_val=ihw
        inner_radius_val=ir
    else
        full_boundary=outer_full
        desymmetrized_full_boundary=outer_desym
        fundamental_boundary=outer_fund
        area=4*outer_half_width*outer_radius+pi*outer_radius^2
        length=sum(crv.length for crv in outer_full)
        length_fundamental=symmetry_accounted_fundamental_boundary_length(outer_fund)

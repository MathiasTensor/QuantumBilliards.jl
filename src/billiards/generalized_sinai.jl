...
function make_quarter_generalized_sinai(half_height::T, half_width::T, theta_right::T, theta_top::T; x0=zero(T), y0=zero(T), rot_angle=zero(T), P1=1.0, P2=1.0) where {T<:Real}
    ...
    right_arc=CircleSegment(rr,angle_right,T(pi)-angle_right-angle_right_pi,hr,kr;orientation= -1) # we need to add it so that the geometry is correct. This is also corrected in the left circle segment in the full boundary analogously

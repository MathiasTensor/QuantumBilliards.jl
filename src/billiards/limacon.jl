...
[`BilliardGeometry.PolarSegment`](@ref). The billiard is symmetric under
...
function LimaconBilliard(ε::T;R::T=one(T),center=SVector{2,T}(zero(T),zero(T))) where {T<:Real}
    ...
    arc=BilliardGeometry.PolarSegment(coef;R=R,arc_angle=T(pi),shift_angle=zero(T),center=center,bc=BilliardGeometry.SpecularReflection(),domain_id=1,segment_id=1)
    ...
    fundamental_domain=BilliardGeometry.PolarDomain{T}(BilliardGeometry.AbsCurve[arc,wall],SVector{2,T}[pR,pL],1)
    full=BilliardGeometry.PolarSegment(coef;R=R,arc_angle=T(2pi),shift_angle=zero(T),center=center,bc=BilliardGeometry.SpecularReflection(),domain_id=1,segment_id=1)

...
function DispersingCircleSegment(R, arc_angle, shift_angle, x0, y0; origin=(zero(x0),zero(x0)),rot_angle=zero(x0), orientation = 1)
...
function VirtualCircleSegment(R, arc_angle, shift_angle, x0, y0; origin=(zero(x0),zero(x0)),rot_angle=zero(x0), orientation = 1)
    ...
    center = SVector(x0,y0)
    L = abs(R*arc_angle)
    return VirtualDispersingCircleSegment(cs,R,arc_angle,shift_angle,center,orientation,L)
end
...
function reversed_circle_segment_domain(pt0, pt1, R, center, orient, x, y)
...
#=    
function domain(circle::L, pt::SVector{2,T}) where {T<:Real,L<:DispersingCircleSegments{T}}

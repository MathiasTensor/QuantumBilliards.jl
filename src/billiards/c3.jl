...
The full physical boundary is one `PolarSegment`. The fundamental domain is
...
function C3Billiard(a::T;scale::T=one(T)) where {T<:Real}
    ...
    full=BilliardGeometry.PolarSegment(coef;R=R,arc_angle=T(2pi),shift_angle=zero(T),center=c,bc=bc,domain_id=1,segment_id=1)
    full_boundary=BilliardGeometry.AbsCurve[full]
    arc=BilliardGeometry.PolarSegment(coef;R=R,arc_angle=T(2pi/3),shift_angle=zero(T),center=c,bc=bc,domain_id=1,segment_id=1)

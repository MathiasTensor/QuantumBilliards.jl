...
"""
function MultiCornerAdaptedFourierBessel(blocks::Vararg{CornerAdaptedFourierBessel,N};symmetries=nothing) where {N}
    ...
    return MultiCornerAdaptedFourierBessel{T,typeof(blocks),typeof(symmetries)}(blocks,dim,offsets,weights,symmetries)
end

"""
    MultiCornerAdaptedFourierBessel(blocks::Tuple;symmetries=nothing)

Construct a multi-corner basis from a tuple of CAFB blocks.
"""
MultiCornerAdaptedFourierBessel(blocks::Tuple;symmetries=nothing)=MultiCornerAdaptedFourierBessel(blocks...;symmetries=symmetries)
...
function resize_basis(basis::MultiCornerAdaptedFourierBessel,billiard::Bi,dim::Int,k) where {Bi<:AbsBilliard}
    ...
    newbasis=MultiCornerAdaptedFourierBessel(blocks;symmetries=basis.symmetries)
    return MultiCornerAdaptedFourierBessel{typeof(newbasis.weights[1]),typeof(newbasis.blocks),typeof(newbasis.symmetries)}(
        newbasis.blocks,newbasis.dim,newbasis.offsets,copy(basis.weights),newbasis.symmetries
    )
end
...
Resize each CAFB block to the explicitly specified dimensions `dims`.
"""
function resize_basis(basis::MultiCornerAdaptedFourierBessel,billiard::Bi,dims::AbstractVector{<:Integer},k) where {Bi<:AbsBilliard}
    ...
    return MultiCornerAdaptedFourierBessel(blocks;symmetries=basis.symmetries)
end
...
Evaluate the derivative with respect to `k` of global basis function `i`.
"""
...
Evaluate the derivatives with respect to `k` of the requested global basis
functions.
"""
@inline function dk_fun(basis::MultiCornerAdaptedFourierBessel,indices::AbstractArray,k::T,pts::AbstractArray;multithreaded::Bool=true) where {T<:Real}
    M=length(pts)
    N=length(indices)
    ...
    @inbounds for block in eachindex(basis.blocks)
        isempty(columns[block])&&continue
        block_values=dk_fun(basis.blocks[block],local_indices[block],k,pts;multithreaded=multithreaded)
        @views dB_dk[:,columns[block]].=block_values
    end
    return dB_dk
end

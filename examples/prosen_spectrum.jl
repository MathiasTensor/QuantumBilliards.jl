...
if !isdir("Prosen/Prosen_Wavefunctions")
    mkdir("Prosen/Prosen_Wavefunctions")
end

if !isdir("Prosen/Prosen_Husimi")
    mkdir("Prosen/Prosen_Husimi")
end

if !isdir("Prosen/Prosen_Momentum")
    mkdir("Prosen/Prosen_Momentum")
end

if !isdir("Prosen/Prosen_Spectra")
    mkdir("Prosen/Prosen_Spectra")
end

if !isdir("Prosen/Prosen_Eigenvalues")
    mkdir("Prosen/Prosen_Eigenvalues")
end
...
billiard, basis = make_prosen_and_basis(a)
#basis = CornerAdaptedFourierBessel(10, Float64(pi/2.0), SVector(0.0,0.0), Float64(0.0)) 

fig = Figure()
ax = Axis(fig[1, 1], aspect = DataAspect())
plot_boundary!(ax, billiard)
save("Prosen/prosen_boundary.png", fig)
...
save("Prosen/prosen_basis_test.png", f)
...
open("Prosen/prosen_solver_logs.txt", "w") do io
    redirect_stdout(io)
...
try
    ...
    save("Prosen/prosen_acc_solver_test.png", f)
catch e
    println(e)
end
...
try
    ...
    save("Prosen/prosen_dm_solver_test.png", f)
catch e
    println(e)
end
...
try
    ...
    save("Prosen/acc_test_state.png", f_acc)
catch e
    println(e)
end
...
try
    ...
    save("Prosen/dm_test_state.png", f_dm) 
catch e
    println(e)
end
...
f = Figure()
ax = Axis(f[1,1], title="Prosen_$(nameof(typeof(basis)))")
...
display(f)
save("Prosen/Prosen_Spectra/prosen_spectrum_$(L1)_$(L2).png", f)
save_numerical_ks_and_tensions!(ks, tens, "Prosen/Prosen_Eigenvalues/prosen_eigenvalues_$(L1)_$(L2).csv")
...
ks, _ = read_numerical_ks_and_tensions("Prosen/Prosen_Eigenvalues/prosen_eigenvalues_$(L1)_$(L2).csv")
...
for i in eachindex(ks) 
    ...
    try
        ...
        save("Prosen/Prosen_Wavefunctions/$(ks[i])_probability.png", f_probability)
        save("Prosen/Prosen_Wavefunctions/$(ks[i])_probability_full.png", f_probability_full)
        save("Prosen/Prosen_Wavefunctions/$(ks[i])_wavefunction.png", f_wavefunction)
        save("Prosen/Prosen_Wavefunctions/$(ks[i])_wavefunction_full.png", f_wavefunction_full)
        save("Prosen/Prosen_Husimi/$(ks[i])_husimi.png", f_husimi)
        save("Prosen/Prosen_Momentum/$(ks[i])_momentum.png", f_momentum)

...
@testset "VerginiSaraceno" begin
    ...
    state_res,_ = compute_spectrum_with_state_scaling_method(
        solver,basis,billiard,k1,k2,dk;
        multithreaded_matrices=true,multithreaded_ks=false
    )
    ks = state_res.ks
...
@testset "BoundaryIntegralMethod local solve" begin
    ...
    k,_ = solve_wavenumber(
        solver,AbstractHankelBasis(),billiard,k_close,0.1
    )
...
@testset "phiFD" begin
    ...
    billiard,_ = make_rectangle_and_basis(w,h)
    fem = QuantumBilliards.FiniteElementMethod(
        billiard,250,250;
        k_max=1000.0,
        offset_x_symmetric=0.1,
        offset_y_symmetric=0.1,
        fundamental=false
    )

    Es,_ = compute_ϕ_fem_eigenmodes(
        fem,phi,5.0,1.01;
        nev=3,maxiter=50000,tol=1e-8
    )
...
@testset "FDM" begin
    ...
    billiard,_ = make_rectangle_and_basis(w,h)
    fem = QuantumBilliards.FiniteElementMethod(
        billiard,400,400;k_max=1000.0
    )

    Es,_ = QuantumBilliards.compute_fem_eigenmodes(
        fem;nev=2,maxiter=100000,tol=1e-8
    )
